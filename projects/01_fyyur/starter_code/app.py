...
def venues():
  ...
  for result in results:
      ...
      venue_object["venues"] = venues_formatted
      data.append(venue_object)
...
def search_venues():
  ...
  results = {}
  search_returned = list(Venue.query.filter(
        Venue.name.ilike(f"%{search_term}%") |
        Venue.state.ilike(f"%{search_term}%") |
        Venue.city.ilike(f"%{search_term}%") 
    ).all())
  
  results["count"] = len(search_returned)
  results["data"] = []
  ...
  for venue in venues:
    ...
    results["data"].append(venue_result)
...
def show_venue(venue_id):
  ...
  for data in past_shows:
      show = {}
      show["artist_name"] = data.artists.name
      show["artist_id"] = data.artists.id
      show["artist_image_link"] = data.artists.image_link
      show["start_time"] = data.start_time.strftime("%m/%d/%Y, %H:%M:%S")
      past_shows_formatted.append(show)
      
  venue["past_shows"]=past_shows_formatted
  venue["past_shows_count"]=len(past_shows)
  
  #data formatting
  upcoming_shows = list(filter(lambda show: show.start_time > datetime.now(), venue.shows))
  upcoming_shows_formatted = []
  for data in upcoming_shows:
      show = {}
      show["artist_name"] = data.artists.name
      show["artist_id"] = data.artists.id
      show["artist_image_link"] = data.artists.image_link
      show["start_time"] = data.start_time.strftime("%m/%d/%Y, %H:%M:%S")
      upcoming_shows_formatted.append(show)
      
  venue["upcoming_shows"]=upcoming_shows_formatted
...
def show_artist(artist_id):
    ...
    for data in past_shows:
        show = {}
        show["venue_name"] = data.venues.name
        show["venue_id"] = data.venues.id
        show["venue_image_link"] = data.venues.image_link
        show["start_time"] = data.start_time.strftime("%m/%d/%Y, %H:%M:%S")

        past_shows_formatted.append(show)

    setattr(artist, "past_shows", past_shows_formatted)
    ...
    setattr(artist, "upcoming_shows", upcoming_shows_formatted)
